%mykernel Kernel computation
%
%              K = MYKERNEL(A,B,KTYPE,P,conparam);
%
% INPUT
%   A,B      Data matrices
%   KTYPE    Kernel type
%   P        Kernel parameter
%   CONPARAM Covariance contribution
%
% OUPUT
...
function K = mykernel(A,B,ktype,kpar,conparam);
...
%         C=[A [B;zeros(n3)]];
%     end 
    K = dd_kernel (A,B,'e',kpar);
    E1=eye(size(K));
    J1=ones(size(K))/length(K);
    Nval = K*(E1-J1)'*K';
%    Nval = cov (K);
    K=(1-conparam)*K+conparam*Nval';
    
case {'mahalanobis', 'mahal'}
    K = dd_kernel (A,B,'e',kpar);
    E1=eye(size(K));
    J1=ones(size(K))/length(K);
    Nval = (E1-J1);
    K=pinv(length(K)*pinv(0.001*eye(size(K))+K)+conparam*Nval');

...
function W = my_inc_add(W,newx,newy,conparam,alreadyadded)
...
if alreadyadded
	...
	switch W.type
        ...
        case 'cosvc'
            K = mykernel(W.x(c,:),W.x(newsetD,:),W.ktype,W.kpar,conparam);
            Kc = (W.y(c)*W.y(newsetD)').*K;
    end
    ...
    c = size(W.x,1);
    newsetD = (1:c)';
    K = mykernel(newx,W.x(newsetD,:),W.ktype,W.kpar,conparam);
%     K = mykernel(newx,newx,W.ktype,W.kpar,conparam);
%     K= K(newsetD,:);
    ...
    while ~done
        ...
        switch situation
            ...
            case 5 % an error becomes a support object
                j = W.setE(nrE_0);
                K = mykernel(W.x(j,:),W.x(newsetD,:),W.ktype,W.kpar,conparam);
                switch W.type
                    case 'svdd'

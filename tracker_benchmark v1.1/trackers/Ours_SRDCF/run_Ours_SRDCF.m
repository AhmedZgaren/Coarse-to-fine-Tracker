% This function runs the  tracker on the video specified in "seq".
% It can be integrated directly in the Online Tracking Benchmark (OTB).
% The parameters are set as in the ISVC 2020 paper.

function results=run_Ours_SRDCF(seq, res_path, bSaveImage, parameters)

% Default parameters used in the ISVC paper
addpath('iCOSVM2.1');
...
params.ThreCOSVM = 0.25;                 % The threshold for iCOSVM Learning
params.threCF = 0.0;                    %the threshold for CF Learning

%iCOSVM parameters
   %CNN
params.net =vgg16; %Backbone for feature extraction
...
params.ktype = 'cov';
params.Kpar = 1000;
params.conparam = 0.1; % between 0 and 1
params.Rayon = 20; % Unit = pixel

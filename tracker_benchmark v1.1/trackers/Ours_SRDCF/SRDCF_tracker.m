% This function implements the SRDCF  +  A. Zgaren et al. tracker
...
for frame = 1:num_frames,
    ...
    if frame > 1
        ...
        pos=CoarseSearch(im,pos,target_sz, pad,net,layer, Rayon,w01); 
        old_pos = inf(size(pos));
        iter = 1;
        % ######## second step fine search   ########################
        %translation search
    ...
    if frame == 1 % first frame INITIALIZATION
        % Initialisation de iCOSVM
        [W1,w01]=initCOSVM(Kpar,ktype,C,conparam,im,target_sz,pos,net,layer, pad);
        %%%%%
        hf_rhs = new_hf_rhs;
        ...
        % ##############  Update mechanism #######################################
        [W1,w01,score]=updateCOSVM(W1,w01,conparam,TL,im,pos,target_sz,pad,net,layer);
        if score > TLCF
...
function [W1,w01]=initCOSVM(Kpar,ktype,C,conparam,imcolor,target_sz,pos,net,layer,pad)
    % Initialize the iCoSVM classifier on the first frame
    %%Input:
    %%%% Kpar, Ktype, C, conparam : parameters for the iCOSVM kernel 
    %%%% imcolor, target_sz, pos, net,layer, pad: parameters for the feature extraction function

    %%Output:
    %%%% W1, w01: Model and weights

    rectf =[pos([2,1])-( target_sz([2,1])*pad/2), target_sz([2,1])*pad];
        
        %target features
        F1=GetFeatures(net,imcolor,layer,rectf,1); % extract features from the first frame
        Fp=GetFeatures(net,imcolor,layer,rectf,10); % extract augmented features from the first frame
        
        %iCoSVM initialization
        a1=target_class(Fp,1);
        x1=+a1;
        y1=getoclab(a1);
        
        W1 = my_inc_setup('cosvc',ktype,Kpar,C,F1,1,conparam); % initialization of the W1(iCoSVM) classifier
         %add targets observation one-by-one : positive examples 
        for i=1:size(x1,1)
         W1 = my_inc_add(W1,x1(i,:),y1(i,1),conparam);
         w01 = inc_store_ahmed(W1,conparam);
        end
        %add Outliers observation one-by-one : negative examples
        for i=1:4
            switch i
                
                case 1 
                    rectn=[rectf(1,1)+rectf(1,3) rectf(1,2) rectf(1,3) rectf(1,4)];
                case 2
                    rectn=[rectf(1,1)-rectf(1,3) rectf(1,2) rectf(1,3) rectf(1,4)];
                case 3
                    rectn=[rectf(1,1) rectf(1,2)+rectf(1,4) rectf(1,3) rectf(1,4)];
                case 4
                    rectn=[rectf(1,1) rectf(1,2)-rectf(1,4) rectf(1,3) rectf(1,4)];
            end

            Fn(i,:)=GetFeatures(net,imcolor,layer,rectn,1);
        y1=-1;
        W1 = my_inc_add(W1,Fn(i,:),y1,conparam);
        w01 = inc_store_ahmed(W1,conparam);

        end
        
end
    function pos=CoarseSearch(im,pos,target_sz, pad,net,layer, Rayon,w01)
        % Calculate the estimated coarse position at a frame t
        %%Inputs:
        %%%% im : image at frame t
        %%%% pos: the target position at frame t-1
        %%%% target_sz : target size
        %%%% pad : padding
        %%%% net : CNN Network for features extraction
        %%%% layer : conv-layer to extract the feature map
        %%%% Rayon : Ray of the search area
        %%%% w01 : weights of the iCOSVM classifier from the frame t-1

        %%Output:
        %%%% pos : the coarse position estimated at frame t  
   
            Lab=1;
             rectf =[pos([2,1])-( target_sz([2,1])*pad/2), target_sz([2,1])*pad];
%             F=GetFeatures(net,im,layer,rectf,1);
%             d1=F*w01;
%             bbx1(1,:)=rectf;
%             ScoreLayer(1,1)=d1(1);
...
%              r=rand(); %generer un nombre reel entre 0 et 1
            
             recti=particule(Rayon,r,rectf); %generer la position de la particule a trame t
             r=r+0.05;
             F=GetFeatures(net,im,layer,recti,1);             
%                  % Scores
                  d1=F(1,:)*w01;
                  ScoreLayer(Lab,1)=d1(1);
%                   if ScoreLayer(Lab,1) > 0
                  bbx1(Lab,:)=recti(1,:);
                  Lab=Lab+1;
%                   end
                             
            end
         [~, T1]=sort(ScoreLayer,'descend');
         N=size(ScoreLayer,1);
         for j=1:N  
             Pos_target_coarse(j,:)=bbx1(T1(j,1),:);    
         end
         
         x_coarse=sum(Pos_target_coarse(1,1));
         y_coarse=sum(Pos_target_coarse(1,2));
         
         rectf=[x_coarse y_coarse ...
                         target_sz([2 1])*pad];
    
          pos=[floor(y_coarse+(target_sz(1,1)*pad/2)) floor(x_coarse+(target_sz(1,2)*pad/2))];
    end

    function [W1,w01,score]=updateCOSVM(W1,w01,conparam,TL,im,pos,target_sz,pad,net,layer)
        % Updates the iCOSVM classifier if the target score is above TL
        %%Input:
        %%%% W1, w01, conparam : Model, weights and covariance parameter
        %%%% TL: Threshold learning
        %%%% im, pos, target_sz, pad, net, layer : parameters for features extraction
        rectf=[pos([2,1]) - (target_sz([2,1])*pad/2), target_sz([2,1])*pad];
        F=GetFeatures(net,im,layer,rectf,1);             
%                  % Scores
                  d1=F*w01;

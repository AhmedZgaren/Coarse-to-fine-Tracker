...
function [precision, fps] = run_tracker_DLT(video, kernel_type, feature_type, show_visualization, show_plots)

	%path to the videos (you'll be able to choose one with the GUI).
	base_path = 'D:\data_seq\OTB100\';
	...
	case 'choose',
		...
		if ~isempty(video),
			[precision, fps] = run_tracker_DLT(video, kernel_type, ...
...
% 		videos(end+1:end+2) = {'Jogging.1', 'Jogging.2'};
		...
		if ~exist('matlabpool', 'file'),
			...
			for k = 1:numel(videos),
				[all_precisions(k), all_fps(k)] = run_tracker_DLT(videos{k}, ...
...
%             for j=0:0.1:0.5
		%call tracker function with all the relevant parameters
		[positions, time] = run_DLT(video_path, img_files, pos, target_sz, ...
	padding, kernel, lambda, output_sigma_factor, interp_factor, cell_size, ...
	features, show_visualization,i,j);
        ...
        save([res_path lower(video) '_ours.mat'], 'results');
        ...
        fprintf('threshold cosvm %1.1f threshold CF %1.1f', i, j)
